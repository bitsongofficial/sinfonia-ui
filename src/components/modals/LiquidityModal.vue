...
const amount2 = computed<string>({
	get() {
		return new BigNumber(props.pool.liquidity)
			.minus(amountInternal.value)
			.toFixed(0)
	},
	set(value) {
		amountInternal.value = new BigNumber(props.pool.liquidity)
			.minus(value)
			.toString()
	},
})
		...
		<div v-if="add">
			<div class="q-mb-21" v-if="pool.coin1">
					...
					<div class="flex items-center no-wrap">
						<div class="text-weight-medium text-right q-mr-16">
							...
							<p class="fs-21 text-no-wrap">
								{{ percentage(pool.coin1.weightPercentage) }} %
							</p>
						</div>
						<PercentageWithImage
				...
				<p class="fs-12 text-dark q-ml-20">
					Available <span class="text-white">10</span> BTSG
				</p>
			</div>
			<div v-if="!single" class="q-mb-21">
				<div class="flex items-center no-wrap q-mb-9" v-if="pool.coin2">
					...
					<div class="flex items-center no-wrap">
						<div class="text-weight-medium text-right q-mr-16">
							...
							<p class="fs-21 text-no-wrap">
								{{ percentage(pool.coin2.weightPercentage) }} %
							</p>
						</div>
						<PercentageWithImage
				...
				<p class="fs-12 text-dark q-ml-20">
					Available <span class="text-white">20</span> 10
				</p>
			</div>
