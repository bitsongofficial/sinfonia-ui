<script setup lang="ts">
	...
	const rules = [
		...
		val => compareBalance(val, props.max ?? '0') || 'You don\'t have enough coins',
	]
...
<template>
	<div>
		<div
			...
			<div :class="'absolute-full rounded-20 bg-primary-darker opacity-50' + (hasError ? ' border-primary' : '')"></div>
			<q-input
				borderless
				v-model="value"
				type="text"
		...
		<p v-if="hasError" class="fs-12 text-primary text-weight-medium">{{errorMessage}}</p>
	</div>
</template>